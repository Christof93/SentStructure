function SentStructure(json_object) {
	...
	this.ready=function(call_b) {
		this.get_node_widths.then(function() {theObj.set_lengths();
		}).then(function() {$("svg.computer").remove();call_b();})
	};	
	...
	this.setBase = function(def_values) {
		...
		if ($(theObj.container).find("#graph-style").length>0) {
			$(theObj.container).find("#graph-style").remove();
		}
		...
		$(theObj.container).find("svg.graph").append(
				$("<style id='graph-style'></style>").text(theObj.getStylesheet())
				);	
	...
	this.getStylesheet = function() {
		return "g.nodes {cursor:pointer;}"+
"g.nodes rect.hov {fill:"+theObj.hover_color+";}"+
...
"text {font-family: Tahoma, Geneva, sans-serif;font-size:"+String(parseInt(theObj.font_size))+"px;pointer-events: none;text-shadow: 0 0.4px 0 rgba(250,250,250,0.3), -0.4px 0 0 rgba(250,250,250,0.3);}";
	...
	this.getNodes = function(sentence,yOffset) {
		...
		ystart=this.top+yOffset*this.ySpacing;
		dist=xstart;
		nodes={};
	...
	this.getAlignments = function() {
		links = [];
		Align_object=this;
		//console.log(Align_object.all_nodes);
	...
	this.drawGraphs = function() {
		...
		sents=Object.keys(theObj.root.segments);	
	...
	this.buildNodes = function(nodes) {
		dep_colors=this.dep_colors;
		nodes=d3.values(nodes);
		that = this;
		dep_up = this.orientationUp;
		//label rectangle
		this.node_label = this.svg.append("g").attr("class","nodes node-plates")
		  ...
		  .enter().append("rect")
			...
			.attr("class",function(d) {
				cl_str="";
				aligned=that.all_nodes[d.id].aligns;
	...
	this.buildLinks = function(links) {
		...
		function linkArc(d) {
			dx = Math.abs((d.target.x+d.target.nodeWidth/2) - (d.source.x+d.source.nodeWidth/2));
			paths={arctop:{x:0,y:0},label:{x:0,y:0}};
			source_y = dep_up? d.source.y : d.source.y+theObj.font_size*2;
			source_x = d.source.x + (d.source.nodeWidth/2);
			d.dep_up=dep_up;
			target_y = dep_up? d.target.y-3 : d.target.y+theObj.font_size*2+3;
			target_x = d.target.x;
			  ...
			  back= dep_up? 0:1;
			  forw= dep_up? 1:0;
			  dist_left=d.target.dists.filter(function(e) {return e>0;}).sort(function(a,b) {return a-b}); 
			  dist_right=d.target.dists.filter(function(e) {return e<0;}).sort(function(a,b) {return b-a});
			  
			  //arrow to left
			if (d.source.x >= d.target.x) {
				place=dist_left.indexOf(dx)+1;
				offset = d.target.nodeWidth - (place*((d.target.nodeWidth/2)/(dist_left.length+1)));
				halfway=-1*Math.abs((target_x+offset-source_x)/2);	
	// 			console.log(d.source.name+" --> "+d.target.name);	
			 ...
			 //arrow to right
				...
				place=dist_right.indexOf((0-dx))+1;
				offset = 0 + (place*((d.target.nodeWidth/2)/(dist_right.length+1)));
				halfway=Math.abs((target_x+offset-source_x)/2);	
	// 			console.log(d.source.name+" --> "+d.target.name);
				paths.arctop.x=source_x+halfway;
		...
		function eliminate_overlap() {
			//console.log(that.path.data())
			that.path.data().sort(function(a,b) {
				distx=a.arc.label.x-b.arc.label.x;
				disty=a.arc.label.y-b.arc.label.y;
				if (Math.abs(distx)<=50 && Math.abs(disty)<=22) {
					if (a.dep_up) {a.arc.label.y-=10;}
					else {a.arc.label.y+=10;}
